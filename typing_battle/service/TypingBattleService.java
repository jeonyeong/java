...
public class TypingBattleService {
	...
	public void playTyping(DeveloperVO dev
			, ArrayList<WordVO> wordList, Scanner sc) {
		...
		for(int i = 0; i < playList.size(); i++) {
			...
			if(inputText.equals(playList.get(i).getWordsword())){
				...
				int amountExp = (int)((10 * playList.get(i).getWordsLevel())
				...
				if(amountExp + dev.getDevExp() >= amountLevel) {
					...
					System.out.println("레벨업!! 현재 레벨 " + dev.getDevLevel());
				}else {
					dev.setDevExp(dev.getDevExp() + amountExp);
				}
				
				// 타이핑 성공시 돈 획득량 amountMoney
				int amountMoney = (int)((10 * playList.get(i).getWordsLevel())
						* (1 + (0.1 * dev.getDevUpgradeMoney())));
